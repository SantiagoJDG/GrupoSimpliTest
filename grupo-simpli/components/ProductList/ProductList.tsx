...
const ProductList: FC<{ products: Motorcycle[] }> = ({ products }) => (
  <Container>
    <MapProductsToCards products={products} />
  </Container>
);

const MapProductsToCards: FC<{ products: Motorcycle[] }> = ({ products }) => {
  return (
    <Row md={4} className="g-4">
      {products.map((product: Motorcycle) => {
        return (
          <Col className="article-list-grid">
            <Card className={styles.card}>
              {product.variants.map((variant: MotorcycleVariants) => {
